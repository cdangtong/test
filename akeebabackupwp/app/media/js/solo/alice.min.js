...
*/
var AKEEBA_ANALYZE_SUCCESS=1,AKEEBA_ANALYZE_WARNING=0,AKEEBA_ANALYZE_FAILURE=-1;if("undefined"==typeof Solo)var Solo={};"undefined"==typeof Solo.Alice&&(Solo.Alice={log_selector:null,analyze:null,raw_output:null,translations:{},akeebaUrl:"",translateUrl:""});Solo.Alice.onLogChange=function(){Solo.Alice.analyze.style.display=Solo.Alice.log_selector.value?"block":none;Solo.Alice.download.style.display=Solo.Alice.log_selector.value?"block":none};
...
Solo.Alice.onAnalyze=function(){if(1==Solo.System.data.get(Solo.Alice.analyze,"started",!1))return!1;Solo.System.addClass(Solo.Alice.analyze,"btn-inverse");Solo.System.removeClass(Solo.Alice.analyze,"btn-primary");Solo.System.data.set(Solo.Alice.analyze,"started",!0);Solo.Alice.log_selector.setAttribute("disabled","disabled");Solo.Alice.raw_output.style.display="none";document.getElementById("stepper-complete").innerHTML="";(new AkeebaStepper({akeebaUrl:Solo.Alice.akeebaUrl,onBeforeStart:function(b){b.data.log=
...
Solo.System.data.set(Solo.Alice.analyze,"started",!1);Solo.Alice.log_selector.removeAttribute("disabled");g&&Solo.Ajax.ajax(Solo.Alice.translateUrl,{data:{keys:JSON.stringify(g)},success:function(a){a=a.match(/###(.*?)###/);if(void 0!==a[1]){a=JSON.parse(a[1]);for(var b="------ BEGIN OF ALICE RAW OUTPUT -----\n",c=0;c<a.length;c++)d=a[c],b+="[b]"+d.check+"[/b]\n"+d.error+"\n\n";b+="------ END OF ALICE RAW OUTPUT -----\n";Solo.Alice.raw_output.querySelector("textarea").innerHTML=b;Solo.Alice.raw_output.style.display=

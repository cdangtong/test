...
abstract class Setup
{
	...
	public static function databaseTypesSelect($selected = '', $name = 'driver')
	{
		...
		foreach ($connectors as $connector)
		{
			...
			if (in_array(strtoupper($connector), array('PDO', 'NONE')))
			{
				continue;
			}
	 ...
	 * @return  string  HTML
	 */
	public static function timezoneSelect($selected = '', $name = 'timezone', $includeDefaults = false)
	{
		$groups      = array();
		$zoneHeaders = array(
			'Africa',
		...
		return Select::groupedList($groups, $name, array(
			'list.attr'   => array(
				'class' => 'form-control akeeba_selectize',
			),
			'id'          => $name,

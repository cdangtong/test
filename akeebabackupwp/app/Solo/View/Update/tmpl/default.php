...
<div id="solo-error-update-nodownloadid" class="alert alert-danger">
	<p>
...
<?php if (!$this->updateInfo->get('loadedUpdate', 1)): ?>
	<div class="alert alert-danger" id="solo-error-update-noconnection">
		<h4>
...
<?php elseif ($this->updateInfo->get('hasUpdate', 0)): ?>
	<div class="alert alert-warning" id="solo-warning-update-found">
		<h4>
			<span class="glyphicon glyphicon-warning-sign"></span>
...
<?php elseif (!$this->updateInfo->get('minstabilityMatch', 0)): ?>
	<div class="alert alert-danger" id="solo-error-update-minstability">
		<h4>
...
<?php elseif (!$this->updateInfo->get('platformMatch', 0)): ?>
	<div class="alert alert-danger" id="solo-error-update-platform-mismatch">
		<h4>
			<span class="glyphicon glyphicon-ban-circle"></span>
...
<?php else: ?>
	<div class="alert alert-success" id="solo-success-update-uptodate">
		<h4>
...
<table class="liveupdate-infotable table table-striped">
	...
	<?php if (!empty($releaseNotes) || !empty($infoUrl)): ?>
		<tr>
			...
			<td>
				<?php if (!empty($releaseNotes)): ?>
					<button href="#" id="btnLiveUpdateReleaseNotes" class="btn btn-link"
                            onclick="Solo.Modal.open({inherit: '#releaseNotesPopup', width: '80%'}); return false;">
						<?php echo Text::_('SOLO_UPDATE_RELEASENOTES') ?>
					</button>
				<?php endif; ?>
				<?php if (!empty($releaseNotes) && !empty($infoUrl)): ?>

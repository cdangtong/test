...
class TestExtract extends BaseObject
{
	...
	public function test_extract($parent)
	{
		...
		{
			$parent->setWarning(Text::_('COM_AKEEBA_ENGINE_TEXTEXTRACT_ERR_PROCESSIMMEDIATELY'));
		...
		{
			$parent->setWarning(Text::_('COM_AKEEBA_ENGINE_TEXTEXTRACT_ERR_INVALIDARCHIVERTYPE'));
		...
		if ($ret['Error'] != '')
		{
			$parent->setError(Text::sprintf('COM_AKEEBA_ENGINE_TEXTEXTRACT_ERR_INTEGRITYCHECKFAILED', $ret['Error']));
	...
	private function loadAkeebaRestore()
	{
		...
		{
			throw new \RuntimeException(Text::_('COM_AKEEBA_ENGINE_TEXTEXTRACT_ERR_ENGINENOTFOUND'), 500);
		}
	}
